...
export function CoveringStyleFields({style,color,onChange,onColorChange}:{style?:CoveringStyle;color:string;onChange:(style:CoveringStyle)=>void;onColorChange:(color:string)=>void}) {
  ...
  return <details className="he-covering-style"><summary>Текстура и штриховка</summary><div className="he-context-fields">
    <label>Текстура<select aria-label="Текстура оболочки" value={value.texture} onChange={e=>edit({texture:e.target.value as CoveringStyle['texture']})}>
      <option value="auto">По типу оболочки</option><option value="none">Без текстуры</option><option value="Rubber002">Резина</option><option value="Fabric061">Ткань</option><option value="Metal049A">Металл</option>
