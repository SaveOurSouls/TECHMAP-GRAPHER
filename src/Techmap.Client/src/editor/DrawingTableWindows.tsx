...
export function DrawingTableWindows(props:{sourceFingerprint?:string;perimeters?:DrawingPerimeters;view?:"e4"|"drawing";document:HarnessDesignDocument;camera:EditorCamera;quantity:number;revision:number;unsaved:boolean;selectedIds:readonly string[];onChange:(d:DrawingDocuments)=>boolean;onCommand:(c:EditorCommand)=>boolean;onReveal:(ids:readonly string[])=>void;wireOptions?:readonly WireDatabaseOption[];onWireSearch?:(query:string)=>void}) {
...
function TableWindow({table,sourceFingerprint,perimeters,camera,document,quantity,revision,unsaved,selectedIds,onChange,onCommand,onReveal,wireOptions,onWireSearch}:Parameters<typeof DrawingTableWindows>[0]&{table:DrawingTable}) {
  ...
  return <section ref={windowRef} className="he-table-window" aria-label={title+" на поле"} style={{...tableWindowStyle(table,camera),...size,transform:`translate(${delta.x}px,${delta.y}px)`}}
    ...
    <div className={`he-table-window-body ${table.kind!=="cut"?"he-table-window-document":""}`}>{table.kind==="cut"?<CutDiagramPanel sourceFingerprint={sourceFingerprint} embedded document={document} quantity={quantity} revision={revision} unsaved={unsaved} relatedIds={selectedIds} onCommand={onCommand} onReveal={id=>onReveal([id])}/>:<DrawingDocumentsPanel sourceFingerprint={sourceFingerprint} unsaved={unsaved} perimeters={perimeters} mode={table.kind} document={document} quantity={quantity} selectedId={null} selectedIds={selectedIds} onChange={onChange} onCommand={onCommand} onReveal={onReveal} wireOptions={wireOptions} onWireSearch={onWireSearch}/>}</div>
