...
function TableWindow({table,camera,document,quantity,revision,unsaved,selectedIds,onChange,onCommand,onReveal}:Parameters<typeof DrawingTableWindows>[0]&{table:DrawingTable}) {
  ...
  return <section ref={windowRef} className="he-table-window" aria-label={title+" на поле"} style={{...tableWindowStyle(table,camera),...size,transform:`translate(${delta.x}px,${delta.y}px)`}}
    ...
    <div className={`he-table-window-body ${table.kind!=="cut"?"he-table-window-document":""}`}>{table.kind==="cut"?<CutDiagramPanel embedded document={document} quantity={quantity} revision={revision} unsaved={unsaved} relatedIds={selectedIds} onCommand={onCommand} onReveal={id=>onReveal([id])}/>:<DrawingDocumentsPanel mode={table.kind} document={document} quantity={quantity} selectedId={null} selectedIds={selectedIds} onChange={onChange} onCommand={onCommand} onReveal={onReveal}/>}</div>
