...
export function PhysicalCoveringsPanel({ document, topology: t, selectedIds, onChange, onReveal, compact=false }: {
  ...
  return <section className="he-relations" aria-label="Оболочки и защита"><details open={!!selected}>
    ...
    {selected && <div className="he-physical-fields">
      ...
      {selected.bundle&&<><label>Укладка<select aria-label="Укладка сохранённой группы" value={selected.bundle.mode} onChange={e=>update({bundle:{...selected.bundle!,mode:e.target.value as 'flat'|'round'}})}><option value="flat">Плоская</option><option value="round">Объёмная</option></select></label><small>Состав: {selected.bundle.members.map(m=>m.kind==='segment'?`S${t.segments.findIndex(s=>s.id===m.id)+1}`:t.coverings?.find(c=>c.id===m.id)?.name??m.id).join(', ')}</small></>}
      ...
      <label>Ширина<InfoHint>0 — плотно по пайпу и нижним покрытиям. Положительное значение задаёт наибольший диаметр; меньшие диаметры меняются по глобальному отношению 1:x. Толщина масштабируется вместе с чертежом.</InfoHint><DraftNumberInput aria-label="Ширина оболочки" min={0} max={10000000} step="any" immediate value={selected.width} onValueChange={width=>update({width})}/></label><label>Цвет<input aria-label="Цвет оболочки" type="color" value={selected.color} onChange={e => update({ color: e.target.value })} /></label>
