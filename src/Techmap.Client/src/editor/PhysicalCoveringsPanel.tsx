...
export function PhysicalCoveringsPanel({ document, topology: t, selectedIds, onChange, onReveal, compact=false }: {
  ...
  return <section className="he-relations" aria-label="Оболочки и защита"><details open={!!selected}>
    ...
    {selected && <div className="he-physical-fields">
      ...
      <label>Ширина<InfoHint>0 — плотно по пайпу и нижним покрытиям. Положительное значение задаёт минимальную ширину. Толщина масштабируется вместе с чертежом.</InfoHint><DraftNumberInput aria-label="Ширина оболочки" min={0} max={10000000} step="any" immediate value={selected.width} onValueChange={width=>update({width})}/></label><label>Цвет<input aria-label="Цвет оболочки" type="color" value={selected.color} onChange={e => update({ color: e.target.value })} /></label>
