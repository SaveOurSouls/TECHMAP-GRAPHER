...
import {materialTexture} from "./editor/texture-tint";
...
export function MaterialPreview({url,tint,angle,scale,lineColor,lineWidth,kind,backgroundColor="#f3f5f6",hatchCode,hatchLineWidth=1}:{kind?:string|null;url:string;tint:string;angle:number;scale:number;lineColor:string;lineWidth:number;backgroundColor?:string;hatchCode?:string|null;hatchLineWidth?:number}){
 ...
 useEffect(()=>{
  ...
  const draw=(image?:HTMLImageElement)=>{
   ...
   canvas.current.width=Math.round(160*ratio);canvas.current.height=Math.round(64*ratio);
   ctx.setTransform(ratio,0,0,ratio,0,0);ctx.clearRect(0,0,160,64);ctx.fillStyle=backgroundColor;ctx.fillRect(0,0,160,64);
   if(hatchCode)drawCatalogHatch(ctx,hatchCode,scale,angle,tint,hatchLineWidth,{x:0,y:0,width:160,height:64});
   else if(image){
    const tile=materialTexture(image,tint,ctx,scale),pattern=ctx.createPattern(tile,"repeat");
 ...
 return <canvas ref={canvas} width={160} height={64} style={{width:160,height:64}} aria-label={hatchCode?"Образец штриховки":"Образец текстуры"}/>;
}
