...
export function MaterialLibrary({config,session}:{config:RuntimeConfig;session:LocalSession}){
 ...
 const add=async()=>{setBusy(true);setError("");try{
  ...
  const m:GlobalMaterial={materialId:crypto.randomUUID(),name:tab==="hatching"?"Новая штриховка":"Новый материал",hatchCode:tab==="hatching"?"H01":null,hatchLineWidth:1,mediaType:"image/png",imageBase64:canvas.toDataURL("image/png").split(",")[1]!,lineColor:"#34434e",lineWidth:1,textureAngle:0,textureScale:1,tint:"#888888",backgroundColor:"#f3f5f6",coveringKind:null,updatedUtc:new Date().toISOString(),revision:0};
  await api.create(m);await load();
 }catch(e){setError(text(e));}finally{setBusy(false);}};
 const [tab,setTab]=useState<"textures"|"hatching">("textures");
 return <section className="material-library" aria-labelledby="materials-title">
  <div className="content-heading"><div><p className="eyebrow">ОБЩАЯ БИБЛИОТЕКА</p><h1 id="materials-title">Материалы <InfoHint>Настройки действуют для новых оболочек во всех проектах. У каждого типа одна основная текстура. Сохранённые оболочки сохраняют свою текстуру и настройки. PNG и SVG — до 10 МиБ; SVG преобразуется в PNG. Текстура — отдельный прозрачный слой: тёмные пиксели задают рисунок, светлые открывают фон. Фон, рисунок и контур имеют отдельные цвета. Толщина линии штриховки не зависит от масштаба сетки.</InfoHint></h1></div>
   <button className="primary-action" type="button" onClick={()=>void add()} disabled={busy}>{tab==="textures"?"+ Материал":"+ Штриховка"}</button></div>
  <nav className="material-tabs" aria-label="Разделы материалов"><button type="button" className={tab==="textures"?"active":""} onClick={()=>setTab("textures")}>Текстуры материалов</button><button type="button" className={tab==="hatching"?"active":""} onClick={()=>setTab("hatching")}>Штриховки</button></nav>
  {error&&<p role="alert">{error}</p>}{busy&&<p role="status">Загрузка…</p>}
  <div hidden={tab!=="textures"} className="material-table-wrap"><table className="material-table" aria-label="Библиотека материалов"><thead><tr><th>Материал / основной тип</th><th>Текстура</th><th>Цвета</th><th>Контур</th><th>Поворот / масштаб</th><th>Действия</th></tr></thead><tbody>
   {rows.filter(row=>!row.hatchCode).map(row=><MaterialRow key={row.materialId} row={row} api={api} reload={load}/>)}</tbody></table></div><div hidden={tab!=="hatching"}><HatchingTable rows={rows.filter(row=>!!row.hatchCode)} api={api} reload={load}/></div>
 </section>;
}
function HatchingTable({rows,api,reload}:{rows:GlobalMaterial[];api:Api;reload:()=>Promise<void>}) { return <div className="material-table-wrap"><table className="material-table" aria-label="Библиотека штриховок"><thead><tr><th>Штриховка / основной тип</th><th>Рисунок</th><th>Цвета</th><th>Контур / линии</th><th>Поворот / масштаб</th><th>Действия</th></tr></thead><tbody>{rows.map(row=><MaterialRow key={row.materialId} row={row} api={api} reload={reload}/>)}</tbody></table><p className="material-catalog-note"><InfoHint>48 штриховок H01–H48 загружены из локального каталога QCAD. Назначение справочное; нормативный знак материала определяется легендой документа.</InfoHint></p></div>; }
function MaterialRow({row,api,reload}:{row:GlobalMaterial;api:Api;reload:()=>Promise<void>}){
 ...
 return <tr><td><label className="sr-only" htmlFor={`material-${row.materialId}`}>Название материала</label><input id={`material-${row.materialId}`} value={draft.name} maxLength={255} onChange={e=>edit({name:e.target.value})} disabled={busy}/>
  ...
  <td><MaterialPreview kind={draft.coveringKind} url={draft.imageBase64?`data:image/png;base64,${draft.imageBase64}`:api.imageUrl(row)} tint={draft.tint} backgroundColor={draft.backgroundColor} hatchCode={draft.hatchCode} hatchLineWidth={draft.hatchLineWidth} scale={draft.textureScale} angle={draft.textureAngle} lineColor={draft.lineColor} lineWidth={draft.lineWidth}/><input ref={input} type="file" accept=".png,.svg,image/png,image/svg+xml" hidden onChange={e=>{const f=e.target.files?.[0];if(f)void upload(f);e.target.value=""}}/>{draft.hatchCode?<label>Узор<select aria-label={`Узор ${draft.name}`} value={draft.hatchCode} onChange={e=>edit({hatchCode:e.target.value})}>{hatchingCatalog.map(h=><option key={h.code} value={h.code}>{h.code} · {h.title}</option>)}</select><InfoHint>{hatchingCatalog.find(h=>h.code===draft.hatchCode)?.description}</InfoHint></label>:<button type="button" className="ui-control" disabled={busy} onClick={()=>input.current?.click()}>Обновить текстуру</button>}</td>
  <td><ColorField label="Цвет фона" value={draft.backgroundColor??"#f3f5f6"} onChange={backgroundColor=>edit({backgroundColor})}/><ColorField label={draft.hatchCode?"Цвет линий":"Цвет текстуры"} value={draft.tint} onChange={tint=>edit({tint})}/></td>
  <td><ColorField label="Цвет контура" value={draft.lineColor} onChange={lineColor=>edit({lineColor})}/><label>Толщина<DraftNumberInput aria-label={`Толщина контура ${draft.name}`} immediate min={.1} max={20} step={.1} value={draft.lineWidth} onValueChange={lineWidth=>edit({lineWidth})}/></label>{draft.hatchCode&&<label>Толщина линий<DraftNumberInput aria-label={`Толщина линий ${draft.name}`} immediate min={.1} max={20} step={.1} value={draft.hatchLineWidth??1} onValueChange={hatchLineWidth=>edit({hatchLineWidth})}/></label>}</td>
