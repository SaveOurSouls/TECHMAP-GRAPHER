...
export function DrawingSelectionProperties({nodes,allNodes=nodes,disabled,change}:{nodes:readonly TemplateNodeV2[];allNodes?:readonly TemplateNodeV2[];disabled:boolean;change:(style:SelectionStyle)=>void}) {
  ...
  const numeric=(label:string,value:number|undefined,min:number,max:number,apply:(value:number)=>void,step=1)=><label>{label}<input type="number" value={value ?? ""} placeholder="Разные" min={min} max={max} step={step} disabled={disabled} onChange={event=>{const value=Number(event.target.value);if(event.target.value!=="" && Number.isFinite(value) && value>=min && value<=max)apply(value);}} /></label>;
