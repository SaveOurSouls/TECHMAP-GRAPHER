...
import { createComponentTemplateApi, type ArticleBinding, type ComponentTemplate, type ComponentTemplateSummary } from "./component-template-api";
...
interface Draft { templateId: string | null; version: number; code: string; name: string; articleBindings: ArticleBinding[]; content: TemplateContent; }
const newDraft = (): Draft => ({ templateId: null, version: 0, code: "", name: "Новый компонент", articleBindings: [], content: newTemplateContent() });
...
export function ComponentLibrary({ config, session }: Props) {
  ...
  async function open(summary: ComponentTemplateSummary) {
    ...
    try {
      ...
      setDraft({ templateId: item.templateId, version: item.version, code: item.code, name: item.name, articleBindings: [...item.articleBindings], content: structuredClone(item.content) });
      setViewId(item.content.views[0]!.id); setSelectedId(null); setUndoStack([]); setError(null); setSaved(null);
    } catch (e) { setError(errorText(e)); } finally { setBusy(false); }
  }
  function startNew() { const next = newDraft(); setDraft(next); setViewId(next.content.views[0]!.id); setSelectedId(null); setUndoStack([]); setError(null); setSaved(null); }
  function changeContent(content: TemplateContent) { setUndoStack(stack => recordTemplateUndo(stack, draft.content, content)); setDraft(current => ({ ...current, content })); setSaved(null); }
  function previewContent(content: TemplateContent) { setDraft(current => ({ ...current, content })); setSaved(null); }
  ...
  const undo = useCallback(() => {
    setUndoStack(stack => {
      ...
      setSelectedId(null); setSaved(null);
  ...
  async function save() {
    ...
    try {
      ...
      const result: ComponentTemplate = draft.templateId ? await api.save(draft.templateId, { expectedVersion: draft.version, ...body }) : await api.create(body);
      setDraft({ templateId: result.templateId, version: result.version, code: result.code, name: result.name, articleBindings: [...result.articleBindings], content: structuredClone(result.content) });
      setSaved(`Сохранена версия ${result.version}`); setError(null); await loadList();
    } catch (e) { setError(errorText(e)); } finally { setBusy(false); }
  }
  ...
  function addBinding() {
    ...
    setBinding(current => ({ ...current, articleKey: "" })); setSaved(null); setError(null);
  }
