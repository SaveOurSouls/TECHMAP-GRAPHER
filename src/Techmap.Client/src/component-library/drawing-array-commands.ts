...
export function withDrawingArticleCounts(content: TemplateContentV3): TemplateContentV3 {
  ...
  const parameters = content.repeaters.filter(domain => domains.has(domain.id) && domain.logicalContactIds.length === 0).map(domain => domain.countParameterId);
  ...
  return { ...content, articleVariants: content.articleVariants.map(article => {
    ...
    return { ...article, parameterValues: [...article.parameterValues.filter(value => !parameters.includes(value.parameterId)), ...parameters.map(parameterId => ({ parameterId, value: count }))] };
  }) };
}

/** Repeats a selected group of drawing primitives without creating electrical contacts. */
export function setDrawingArray(content: TemplateContentV3, viewId: string, layerId: string, selectedIds: readonly string[], input: DrawingArrayInput, existingDomainId?: string): TemplateContentV3 {
  ...
  if (!placement) {
    const nodes = layer.nodes.filter(n => selectedIds.includes(n.id));
    ...
    placement = {repeatDomainId:domainId,prototypeGroupId:groupId,step:{x:c(input.pitchX),y:c(input.pitchY)},contactPointIds:[]};
    view.repeatPlacements.push(placement);
  }
  if (placement.contactPointIds.length) throw new Error("Выберите отдельный графический массив, без электрических контактов.");
