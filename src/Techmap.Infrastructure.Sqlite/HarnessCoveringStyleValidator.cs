...
internal static class HarnessCoveringStyleValidator
{
    ...
    public static void Validate(JsonElement style)
    {
        ...
        if (style.TryGetProperty("hatchCode", out var code) && (code.ValueKind != JsonValueKind.String || !(code.GetString() == "" || System.Text.RegularExpressions.Regex.IsMatch(code.GetString()!, "^H(0[1-9]|[1-3][0-9]|4[0-8])$")))) throw Invalid();
        ...
        foreach (var key in new[] { "lineColor", "hatchColor", "textureTint" })
        ...
        foreach (var (key, min, max) in new[] { ("textureScale", .1, 10d), ("textureRotation", -180d, 180d), ("hatchSpacing", 1d, 100d), ("hatchRotation", -180d, 180d), ("lineWidth", .1, 20d), ("hatchLineWidth", .1, 20d) })
