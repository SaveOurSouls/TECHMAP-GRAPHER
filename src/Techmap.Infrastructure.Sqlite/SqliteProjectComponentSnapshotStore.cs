    ...
    TimeProvider timeProvider) : IProjectComponentSnapshotStore
{
    ...
    {
        if (source.TemplateId == Guid.Empty || source.Version <= 0 || source.SchemaVersion != 3)
            throw Invalid("component_template_version_invalid", "Only a published component template version 3 can be placed.", "sourceTemplateId");
